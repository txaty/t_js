...
static uint8_t vtype(jsval_t v) { return is_nan(v) ? (v >> 19) & 15 : JS_NUM; }
static unsigned long vdata(jsval_t v) { return v & ~((jsval_t)0xfff80000); }
static jsval_t mkcoderef(jsval_t off, jsoff_t len) { return mkval(JS_CODEREF, (off & 0xfff) | ((len & 127) << 12)); }
...
static uint8_t vtype(jsval_t v) { return is_nan(v) ? ((v >> 48) & 15) : (uint8_t)JS_NUM; }
static unsigned long vdata(jsval_t v) { return (unsigned long)(v & ~((jsval_t)0x7fff << 48)); }
static jsval_t mkcoderef(jsval_t off, jsoff_t len) { return mkval(JS_CODEREF, (off & 0xffffff) | ((len & 0xffffff) << 24)); }
...
static bool is_err(jsval_t v) { return vtype(v) == JS_ERR; }
static bool is_op(uint8_t tok) { return tok >= JS_OP_DOT; }
static bool is_unary(uint8_t tok) { return tok >= JS_OP_POSTINC && tok <= JS_OP_UMINUS; }
static bool is_right_assoc(uint8_t tok) { return (tok >= JS_OP_NOT && tok <= JS_OP_UMINUS) || (tok >= JS_OP_Q && tok <= JS_OP_OR_ASSIGN); }
static bool is_assign(uint8_t tok) { return (tok >= JS_OP_ASSIGN && tok <= JS_OP_OR_ASSIGN); }
...
static jsval_t upper(struct js *js, jsval_t scope) { return mkval(JS_OBJ, loadoff(js, vdata(scope) + sizeof(jsoff_t))); }
...
static void ffiinitcbargs(union ffi_val *args, jw_t w1, jw_t w2, jw_t w3, jw_t w4, jw_t w5, jw_t w6)
{
  args[0].w = w1;
  args[1].w = w2;
  args[2].w = w3;
...
static jsval_t js_expr(struct js *js, uint8_t etok, uint8_t etok2)
{
  ...
  uint8_t tok, ops[JS_EXPR_MAX], pt = JS_TK_ERR; // operator indices
